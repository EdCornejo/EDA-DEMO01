...
func Dial(addr string) (t Telnet, err error) {
	...
	if err == nil {
		...
		t.timeout = time.Second * 5 // default is 5
		fmt.Println("comunicacion establecida") // muestra la conexion
		fmt.Println(t.conn)// muestra los valores enviados por el servidor
		fmt.Println("valores leidos")
		fmt.Println(t.reader)// muestra los valores leidos
	}//else {	fmt.Println("Descripcion del Error\n")
	//	fmt.Println(err)}
	return
}
...
func (t Telnet) Read(expect string) (str string, err error) {
	...
	for {
		...
		fmt.Println(b,e)//Imprimir buf.String
		if e != nil {
			err = e
			break
		}
		...
		if strings.Contains(buf.String(), expect) {
			str = buf.String()
			fmt.Println(str)//Imprimir buf.String
			break
		}
	}
